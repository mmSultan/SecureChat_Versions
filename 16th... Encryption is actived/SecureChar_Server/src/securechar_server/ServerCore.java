        ...
        try {
            ...
            switch(rcvPkt.getType()){
                case 1: // Authintication
                    ...
                    if(rcvPkt.getData2().equals(USER.getPass())){  // if pass is true
                        sendPkt.setData1("VALID");   
                        sendPkt.setOnlineList(Queue.onlineList); // add the whole online list
                        sendPkt.setPublicKey(Info.getServerPublicKey());// send server Public key to the client 
                        
                        Queue.addMainSocket(clientID, clientSocket);
                        Queue.addOnline(clientID);
                        Queue.addPublikKeysList(clientID, rcvPkt.getPublicKey()); // add the public key to server records
                ...
                case 2: //DATA
                    ...
                    String tempMsg =rsa.decrypt(rcvPkt.getMsgOrFile()); // decrupt the message to be encrpted by the destination client publick key         
                   ...
                   // send the text packet to all the users in the destinationList
                    for(String destID : rcvPkt.getDestIDList()){
                        ...
                        sendPkt.setMsgOrFile(rsa.encrypt(tempMsg, Queue.publikKeysList.get(destID))); // message encrypted by the destination client public key
                        
//                        System.out.println("MSg before: "+rcvPkt.getFile());
//                        System.out.println("Msg After: "+rsa.decrypt(rcvPkt.getFile()));
                ...
                case 5://5-UpdatechatList,
                    ...
                    }else if(!rcvPkt.getData1().equals(rcvPkt.getClientID())) { // it is remove pkt
                        sendPkt = new Packet();
                        sendPkt.setType((short)5);
                        sendPkt.setClientID(rcvPkt.getData1());// get the ID of invited user
                        sendPkt.setSessionID(sessionID);
                ...
                case 7:// file transfare
                    if(rcvPkt.isCreateFileSocket()){
                        ...
                        for(String destID : rcvPkt.getDestIDList()){
                            ...
                            sendPkt.setSessionID(rcvPkt.getSessionID());
                            sendPkt.setData1(rcvPkt.getData1());
                            sendPkt.setMsgOrFile(rcvPkt.getMsgOrFile());
                            new SenderThread(sendPkt).start();
                ...
                case 8://receiving chat history from client
                    ...
                    break;
                case 9:
                    new SenderThread(rcvPkt).start(); // this is a special case that
                    // the sender thread will process in order not to fill up the client with files.
